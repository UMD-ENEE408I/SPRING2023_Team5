...
def send(theta, m_address, m_port):
	UDPServerSocket.sendto(theta, (m_address, m_port))
...
def locate():
	...
	while receivee.decode() != "1" and micee.decode() != "mice1":
		...
		(receivee, addr) = UDPServerSocket.recvfrom(1024)

		print("Receive:")
		print(receivee.decode())
		print("MICE:")
		print(micee.decode())

		if(receivee.decode() == "1" and micee.decode() == "mice1"):
			print("***STEP 1***")


	while msg.decode() != "1" and msgg.decode() != "5":
		send(state, address[0], port[0])
		(msg, addr) = UDPServerSocket.recvfrom(1024)
		send(initialize, address[0], port[0])
		(msgg, addr) = UDPServerSocket.recvfrom(1024)
		
		if msg.decode() == "1" and msgg.decode() == "5":
			print("***STEP 2***")
